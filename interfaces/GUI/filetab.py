...
class hiveTreeModel(QAbstractItemModel):
    ...
    def __init__(self, fileViewTab, ref_obj, gui, fileid, filepath, parent=None):
        ...
        self.tapi     = ref_obj.gui.RD.hivebrowser.tapi
        self.gui      = gui
        self.fileid   = fileid
    ...
    def list_values(self, node):
        ...
        self.filetab.valueTable.setHorizontalHeaderLabels(["Name", "Type", "Value"])
...
class filetab:
    ...
    def viewTree(self, fileids=[]):
        if not fileids:
            fileids = self.gui.RD.hivebrowser.get_current_fileids() 
            # user did not select a file
            if not fileids:
                return

        for fileid in fileids:
            filepath = self.gui.RD.hivebrowser.get_file_path(fileid)
