...
class TemplateManager:
    def __init__(self, template_directory="template_files"):
        self.template_directory = os.path.join("registrydecoder", "templates", template_directory)
        self.templates = []    
    ...
    # Returns the list of loaded templates which run on the specified hive 
    # ("System", "Software" ...)
    def get_hive_templates(self, hive):
        return [t for t in self.templates if hive in t.hives]
   
    def make_copy(self):
        copy             = TemplateManager()
        copy.header      = self.header
        copy.report_data = self.report_data
        copy.error_set   = self.error_set
    ...
    def set_report_header(self, header, is_header=1):
    ...
    def set_report_data(self, report_vals):
