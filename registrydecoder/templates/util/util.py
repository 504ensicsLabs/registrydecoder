...
class templateutil:
    ...
    def key_name(self, node):
        return self._get_tree().stringtable.idxtostr(node.sid)  
    ...
    def root_path_node(self, path):
        ...
        return self._get_tree().check_path_from_root(path, self.current_fileid())
    ...
    def get_name_data(self, path, namestr):
        ...
        if node:
            ret = self._get_tree().obj.vtable.key_name(node[-1], namestr, self.current_fileid())
    ...
    def get_value_for_node_name(self, node, namestr):
        ...
        ret = self._get_tree().obj.vtable.key_name(node, namestr, self.current_fileid())
    ...
    def get_node_value_data(self, node, namestr, valstr): 
        ...
        ret = self._get_tree().obj.vtable.key_name_value(node, namestr, valstr, self.current_fileid())
    ...
    def path_subkeys(self, path):
        ...
        subkeys = self._get_tree().walk_children_path(path_list, self.current_fileid(), 1)
    ...
    def root_node(self):
        return self._get_tree().rootnode(self.current_fileid()[0])
    ...
    def get_children_fid(self, node, depth=1):
        return self._get_tree().walk_children(node, self.current_fileid(), depth)
    ...
    def idxtonode(self, idx):
        return self._get_tree().idxtonode(idx)
    
    def values_for_node(self, node):
        return self._get_tree().obj.vtable.values_for_node(node, self.current_fileid())
    
    def stringid(self, sid):
        return self._get_tree().stringtable.idxtostr(sid)  

    # returns the nodes from 'node' to root of tree
    def node_to_root(self, node):
        nodes =  self._get_tree().walk_node_to_root(node)
    ...
    def node_searchfor(self, searchfor, partial=0):
        ...
        # returns a generator
        nodes = self._get_tree().node_searchfor(searchfor, self.current_fileid(), partial)
    ...
    def names_for_search(self, searchfor, partial):
        ...
        if partial:
            nodevals = self._get_tree().obj.vtable.names_for_search_partial(searchfor, self.current_fileid())
        else:
            nodevals = self._get_tree().obj.vtable.names_for_search(searchfor, self.current_fileid()) 
    ...
    def data_for_search(self, searchfor, partial):
        ...
        if partial:
            nodevals = self._get_tree().obj.vtable.data_for_search_partial(searchfor, self.current_fileid())
        else:
            nodevals = self._get_tree().obj.vtable.data_for_search(searchfor, self.current_fileid()) 
    ...
    def process_nodevals(self, nodevals):
        ...
        for nodeval in nodevals:
            ...
            nodeval.node          = self._get_tree().idxtonode(nodeval.nodeid)
